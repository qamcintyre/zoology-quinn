...
#                     name=f"repeat-attention-dim-{d_model}-heads-{num_heads}-layers-{num_layers}-reps-{num_reps}",
#                     **model_factory_kwargs
#                 )
#                 models.append(model)

# conv repeated attention
for d_model in [
#    8, 
    16, 
    32
...
#        6,
        ...
        for num_layers in [
            ...
            attention_mixer = dict(
                name="zoology.mixers.recurrent_attn_explore.mha_conv.MHC",
                kwargs={
                    "dropout": 0.1,
                    "num_heads": num_heads
                },
            ...
            model = ModelConfig(
                ...
                sequence_mixer=mixer,
                max_position_embeddings=0,
                name=f"conv_attn-dim-{d_model}-heads-{num_heads}-layers-{num_layers}",
                **model_factory_kwargs
            )
            models.append(model)
    ...
    for model in models:
        for i, lr in enumerate(np.logspace(-3.5, -2, 4)):
            ...
            config = TrainConfig(
                ...
                max_epochs=32,
                logger=LoggerConfig(
                    project_name="CurriculumParityTokenMixed",
